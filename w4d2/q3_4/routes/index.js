...
router.post("/addCart", (req, res) => {
  ...
  req.session.totalPrice = cart.reduce((acc, x) => acc + (x.price * x.quantity), 0);
  req.session.cart = cart;
  console.log(cart);
  res.send({cartItemCount: cart.length});
});

router.get("/shoppingcart", (req, res) => {
  res.render('shoppingcart.ejs', { cart: req.session.cart, totalAmt: req.session.totalPrice });
})
